...
dbt build --select source_status:fresher+: build all models that have fresher information compared to a previous job/state (in dbt cloud you can refer to a previous job run with the "Compare changes against" option when configuring a new job. This will look at sources.json of that previous job which can be found in the "artifacts"-tab.)\
